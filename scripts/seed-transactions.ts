...
const recipientNames = [
  ...
  'Ustadz Luthfi Assyaukanie',
  'Ustadz Quraish Shihab',
  'Ustadz Din Syamsuddin',
  'Ustadz Arifin Ilham',
  ...
  'Panitia Zakat RT 05',
  'Takmir Masjid Baitul Hikmah',
  'Amil Zakat Kelurahan',
  'Ketua RW 03',
  'Bendahara Masjid',
  'Sekretaris Takmir',
  'Koordinator Zakat',
...
const zakatNotes = {
  [ZakatType.FITRAH]: [
    'Zakat fitrah keluarga',
    'Zakat fitrah anak',
    'Zakat fitrah istri',
    'Zakat fitrah suami',
    'Zakat fitrah orang tua',
    'Zakat fitrah kakek nenek',
    'Zakat fitrah cucu',
    'Zakat fitrah mertua',
    'Zakat fitrah ART',
    'Zakat fitrah sopir',
  ...
  [ZakatType.INFAK]: [
    ...
    'Infaq palestina',
    'Infaq rohingya',
    'Infaq buku perpustakaan',
    'Infaq beasiswa',
    'Infaq air bersih',
    'Infaq listrik masjid',
  ],
  [ZakatType.OTHER]: [
    'Sedekah jariyah',
    'Wakaf tanah',
...
const familyNames = [
  ...
  'Keluarga Bu Lurah',
  'Keluarga Ustadz',
  'Keluarga Dokter Ahmad',
  'Keluarga Pak Camat',
  'Keluarga Bu Guru',
  'Keluarga Haji Umar',
  'Keluarga Pak Kades',
  'Keluarga Bu Bidan',
  'Keluarga Almarhum Kakek',
  'Keluarga Nenek',
  'Keluarga Pak Kyai',
  'Keluarga Bu Nyai',
  'Keluarga Almarhum Ayah',
  'Keluarga Almarhumah Ibu',
]
...
async function seedRandomTransactions(count: number = 100) {
  console.log(
    `🚀 Starting to generate ${count} random zakat transactions with equal distribution...`
  )
  console.log(
    `📅 Date range: ${new Date(Date.now() - 3 * 365 * 24 * 60 * 60 * 1000).toDateString()} to ${new Date().toDateString()}`
  )
  console.log('')
  ...
  console.log(
    `📊 Target distribution: ${itemsPerType} transactions per type (remainder: ${remainder})`
  )
  console.log('')
  ...
  for (let i = 0; i < count; i++) {
    ...
    const onBehalfOfCount =
      onBehalfOf.length > 0 && onBehalfOf[0].name.includes('orang')
        ? parseInt(onBehalfOf[0].name.match(/\((\d+) orang\)/)?.[1] || '1')
        : 1
    ...
    const combinedNotes =
      paymentMethod === PaymentMethod.CASH ? zakatNote : `${zakatNote} - ${paymentNote}`
  ...
  const amountStats = {
    fitrah: dummyData
      .filter(t => t.zakatType === ZakatType.FITRAH)
      .reduce((sum, t) => sum + t.amount, 0),
    mal: dummyData.filter(t => t.zakatType === ZakatType.MAL).reduce((sum, t) => sum + t.amount, 0),
    infaq: dummyData
      .filter(t => t.zakatType === ZakatType.INFAK)
      .reduce((sum, t) => sum + t.amount, 0),
    other: dummyData
      .filter(t => t.zakatType === ZakatType.OTHER)
      .reduce((sum, t) => sum + t.amount, 0),
  }
  ...
  console.log(
    `   • Fitrah: ${stats.fitrah} transactions (${((stats.fitrah / count) * 100).toFixed(1)}%) - Avg: Rp ${stats.fitrah > 0 ? Math.round(amountStats.fitrah / stats.fitrah).toLocaleString('id-ID') : '0'}`
  )
  console.log(
    `   • Mal: ${stats.mal} transactions (${((stats.mal / count) * 100).toFixed(1)}%) - Avg: Rp ${stats.mal > 0 ? Math.round(amountStats.mal / stats.mal).toLocaleString('id-ID') : '0'}`
  )
  console.log(
    `   • Infaq: ${stats.infaq} transactions (${((stats.infaq / count) * 100).toFixed(1)}%) - Avg: Rp ${stats.infaq > 0 ? Math.round(amountStats.infaq / stats.infaq).toLocaleString('id-ID') : '0'}`
  )
  console.log(
    `   • Other: ${stats.other} transactions (${((stats.other / count) * 100).toFixed(1)}%) - Avg: Rp ${stats.other > 0 ? Math.round(amountStats.other / stats.other).toLocaleString('id-ID') : '0'}`
  )
  console.log('')
  console.log('💳 Payment Methods:')
  console.log(
    `   • Cash: ${paymentStats.cash} transactions (${((paymentStats.cash / count) * 100).toFixed(1)}%)`
  )
  console.log(
    `   • Bank Transfer: ${paymentStats.bankTransfer} transactions (${((paymentStats.bankTransfer / count) * 100).toFixed(1)}%)`
  )
  console.log(
    `   • E-Wallet: ${paymentStats.eWallet} transactions (${((paymentStats.eWallet / count) * 100).toFixed(1)}%)`
  )
  console.log(
    `   • Other: ${paymentStats.other} transactions (${((paymentStats.other / count) * 100).toFixed(1)}%)`
  )
  console.log('')
  ...
  console.log(
    `   • Min Amount: Rp ${Math.min(...dummyData.map(t => t.amount)).toLocaleString('id-ID')}`
  )
  console.log(
    `   • Max Amount: Rp ${Math.max(...dummyData.map(t => t.amount)).toLocaleString('id-ID')}`
  )
  console.log('')
