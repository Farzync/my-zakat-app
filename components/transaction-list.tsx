...
export function TransactionList() {
  ...
  const getZakatTypeColor = (type: ZakatType) => {
    const baseClasses = 'ml-2 rounded-full px-2.5 py-0.5 text-sm font-medium transition-colors'
      ...
      <CardHeader>
        ...
        <div className="flex flex-col sm:flex-row gap-4 mt-4 items-center">
          ...
          <Button
            onClick={fetchData}
            variant="outline"
            className={`p-2 h-10 w-10 hover:bg-slate-200 text-slate-600 hover:text-slate-800 transition-all`}
            aria-label="Refresh"
            disabled={loading}
          >
            <RefreshCw
              className={`h-5 w-5 ${loading ? 'animate-spin' : ''} transition-transform duration-300`}
            />
          </Button>
                          ...
                          <AlertDialogContent>
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription>
                                ...
                                {transaction.donorName}? Tindakan ini tidak dapat dibatalkan.
                              </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
